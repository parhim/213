  ...
  @Override protected void execute () throws InvalidInstructionException, MachineHaltException, RegisterSet.InvalidRegisterNumberException, MainMemory.InvalidAddressException
  {
    ...
    switch (insOpCode.get()) {
      ...
      case 0x1: // ld o(rs), rd .......... 1psd  (p = o / 4)
          data = mem.readInteger(i0*4+reg.get(i1));
        reg.set(i2,data);
        break;
      case 0x2: // ld (rs, ri, 4), rd .... 2sid
          data = mem.readInteger(reg.get(i0)+reg.get(i1)*4);
          reg.set(i2,data);
        break;
      case 0x3: // st rs, o(rd) .......... 3spd  (p = o / 4)
          data = reg.get(i0);
          mem.writeInteger(i1*4+reg.get(i2),data);
        break;
      case 0x4: // st rs, (rd, ri, 4) .... 4sdi
          data = reg.get(i0);
          mem.writeInteger(reg.get(i1)+reg.get(i2)*4,data);
        break;
      case 0x6: // ALU ................... 6-sd
	switch (insOp0.get()) {
	  ...
	  case 0x1: // add rs, rd ........ 61sd
          reg.set(i2,reg.get(i1) + reg.get(i2));
	    break;
	  case 0x2: // and rs, rd ........ 62sd
          reg.set(i2,reg.get(i1) & reg.get(i2));
	    break;
	  case 0x3: // inc rr ............ 63-r
          reg.set(i2,reg.get(i2)+1);
	    break;
	  case 0x4: // inca rr ........... 64-r
          reg.set(i2,reg.get(i2)+4);
	    break;
	...
	break;
      case 0x7: // sh? $i,rd ............. 7dii
        if (insOpImm.get()<0){
            reg.set(i0,reg.get(i0) >> -insOpImm.get());
        } else {reg.set(i0,reg.get(i0) << insOpImm.get());}
        break;
